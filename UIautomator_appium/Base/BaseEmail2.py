...
def send_mail(**kwargs):
    ...
    if kwargs.get("report", "0") != "0":
        ...
        msg.attach(part)
    #此种方法无法登录企业认证，只可以QQ使用
    # server = smtplib.SMTP_SSL(smtp_server, kwargs["port"])
    server = smtplib.SMTP()
    server.connect(smtp_server, kwargs["port"])
    server.ehlo()
    server.starttls()
...
if __name__ == '__main__':
    to_addr = ["lubaobao_v@didichuxing.com"]
    #测试didi邮箱
    ...
    send_mail(to_addr = to_addr, mail_host = mail_host, mail_user=mail_user, port=port, mail_pass=mail_pass, header_msg=header_msg, report=report, attach=attach, report_name="接口测试报告.png")
