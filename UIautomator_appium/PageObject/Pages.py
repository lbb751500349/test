...
class PagesObjects:
    ...
    def __init__(self, kwargs):
        ...
        # 解决selenium.WebDriverException: Unable to launch the app: Error: Trying to start logcat capture but it's already started!（85行）
        ...
        self.get_value = []
        # 检查点特殊标志，结合get_value使用。若为真，说明检查点要对比历史数据和实际数据
        self.is_get = False
        self.msg = ""
    ...
    def checkPoint(self, kwargs={}):
        ...
        if self.test_msg[0] is not False:  # 如果用例编写正确
            if result is not True and be.RE_CONNECT:
                ...
                # 解决selenium.WebDriverException: Unable to launch the app: Error: Trying to start logcat capture but it's already started!（27行）
                # self.driver.launch_app()
